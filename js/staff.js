...
/*
  [staff] v40.0 — canonical: businesses.defaultHours + staff subcollection (hoursOverride)
  ...
  - AO mirror (optional): step8.staff & step8.staff_hours (TR) — kept for backward UI pieces
*/
...
function rebuildStaffFromDocsPreserveSelection(){
  ...
  staff = staffDocs.map(rec=>{
    ...
    const norm = defaultHoursToUI(eff); // convert to UI hours

    const isOwner = !!(data.uid && OWNER_UID && data.uid === OWNER_UID) || (!!OWNER_UID && rec.id === OWNER_UID);
    return {
      id: rec.id,
      name: data.name || "Çalışan",
      isOwner,
      role: data.position || data.role || "Personel",
  ...
  // Eğer admin listede yoksa, estetik olarak en başa sanal bir satır eklemiyoruz;
  // finalizeOnboarding owner doc'u zaten oluşturuyor olmalı.
...
async function clampAllStaffToBusinessHours(newBH){
  // staff (UI) üzerinde clamp → hoursOverride hesapla → sub doc güncelle
  const batch = writeBatch(db);
  let anyChange = false;
  const messages = [];
  ...
  if (!anyChange){
    // sadece override normalize etmiş olabilir, yine de yazalım
