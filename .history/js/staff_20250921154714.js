...
console.log("[staff] v31.3 — businesses kanonik + AO mirror, BH clamp + realtime + UI rehydrate (hours>workingHours fix)");
...
/* Firestore upsert: önce update, olmazsa merge set */
...
/* Otomatik sıkıştırma tercihi */
const AUTO_CLAMP = true; // istersen localStorage ile yönet: JSON.parse(localStorage.getItem('autoClampBH') ?? 'true')
...
function normalizeDay(src){
  ...
  // DİKKAT: src.open boolean olabilir; saat stringleri start/from vb.
  const start = src.start || src.from || src.begin || src.startTime || '09:00';
  const end   = src.end   || src.to   || src.finish || src.endTime   || '18:00';
...
// ÖNEMLİ DÜZELTME: 'hours' > 'staffHours' > 'workingHours' > 'weeklyHours'
function normalizeAnyHours(src){
  const raw = src?.hours || src?.staffHours || src?.workingHours || src?.weeklyHours || src || {};
