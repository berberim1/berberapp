/* ===========================
   AYARLAR – Firestore Entegre (v11.7)
   - upload fallback (updateDoc → setDoc)
   - lightbox (thumb modal)
   - bucket healthcheck log
   - UID tekil
   - owner.name öncelikli, adres root businessLocation + step5 mirror
   =========================== */

console.log("[settings] settings.js yüklendi (v11.7)");
...
function setDirty(v = true) {
  ...
  if (b) { b.disabled = !DIRTY; b.classList.toggle("is-dirty", DIRTY); }
}
...
(async () => {
  try {
    ...
    if (nav) {
      ...
      if (!holder) { holder = document.createElement("div"); holder.id = "bar-menu"; document.body.insertAdjacentElement("afterbegin", holder); }
    ...
    const openBtn = document.querySelector("nav.rail #openProfile") || document.querySelector("nav.rail .rail__bottom .avatar-btn");
    const overlay = document.getElementById("sbOverlay");
    const closeBtn = document.getElementById("sbClose");
    const openSb = () => { overlay?.classList.add("show"); document.body.style.overflow = "hidden"; };
    const closeSb = () => { overlay?.classList.remove("show"); document.body.style.overflow = ""; };
    ...
    sbLogout?.addEventListener("click", async () => {
      try { sbLogout.disabled = true; sbLogout.textContent = "Çıkış yapılıyor…"; await signOut(auth); }
      catch (err) { console.error("signOut error:", err); }
      finally { const ret = encodeURIComponent("settings.html"); location.href = `admin-register-login.html#login?return_to=${ret}`; }
    });
  } catch (e) {
...
const DATA = {
  İstanbul: {
    Ümraniye: ["Atakent","Atatürk","Aşağıdudullu","Çakmak","Esenevler","Esenkent","Hekimbaşı","Ihlamurkuyu","İnkılap","İstiklal","Kazım Karabekir","Namık Kemal","Parseller","Sarıkaya","Tantavi","Tatlısu","Tepeüstü","Yamanevler","Yukarıdudullu"],
    Üsküdar: ["Altunizade","Aziz Mahmut Hüdayi","Bahçelievler","Beylerbeyi","Burhaniye","Bulgurlu","Cumhuriyet","Çengelköy","Ferah","Güzeltepe","İcadiye","Kandilli","Kısıklı","Kuzguncuk","Küçük Çamlıca","Küçüksu","Mehmet Akif Ersoy","Mimar Sinan","Salacak","Selami Ali","Sultantepe","Ünalan","Valide-i Atik","Yavuztürk","Zeynep Kamil"],
  },
};
...
function fillDistricts(){ if(!dist || !hood) return; dist.length=0; dist.add(new Option("İlçe seçin","",true,true)); dist.disabled=false; hood.length=0; hood.add(new Option("Mahalle seçin","",true,true)); hood.disabled=true; Object.keys(DATA["İstanbul"]).forEach((d)=>dist.add(new Option(d,d))); }
function fillHoods(){ if(!hood) return; hood.length=0; hood.add(new Option("Mahalle seçin","",true,true)); hood.disabled=!dist.value; if(!dist.value) return; (DATA["İstanbul"]?.[dist.value]||[]).forEach((h)=>hood.add(new Option(h,h))); }
function ensureOption(select,value,label){ if(!select||!value) return; const exists=[...select.options].some((o)=>o.value===value); if(!exists) select.add(new Option(label??value,value)); select.value=value; }
...
/* — Lightbox CSS’ini enjekte edelim — */
...
async function uploadImageToStorage(kind, file, uid) {
  ...
  console.log("[upload target]", r.toString()); // gs://.../uploads/UID/...

  // 1) Storage’a yükle
  await uploadBytes(r, file, { contentType: file.type || "application/octet-stream" });
  // 2) URL al
  const url = await getDownloadURL(r);
  // 3) Firestore’a yaz (fallback: setDoc)
  ...
  try {
    ...
    if (e?.code === "not-found" || /No document to update/i.test(e?.message || "")) {
      await setDoc(
        ref,
        { images: { [kind]: [url] }, updatedAt: serverTimestamp() },
        { merge: true }
      );
    } else {
...
function materializeData(d) {
  // (YENİ) root businessLocation > (ESKİ) step5.businessLocation sırasıyla
  ...
  // İşletme hakkında/telefon (owner’dan ayrı)
  const about = d?.business?.about || "";
  const phone = d?.business?.phone || step2.phone || "";

  // Hizmetler: root services + step7.services birleşik & uniq
...
async function prefillFromDoc(d) {
  ...
  // Sidebar profil adı: owner.name > step2.adminName
...
function buildUpdatesForFirestore() {
  ...
  // (ESKİ) mirror: step5.businessLocation  — geriye dönük uyumluluk
  ...
  // İşletme bilgileri (owner’dan ayrı tutulur)
