/* ===========================
   AYARLAR – Firestore + Business Hours MODAL (v12.2)
   =========================== */

console.log("[settings] settings.js yüklendi (v12.2 modal)");
...
const openHoursBtn = $("#openHoursBtn");
const bhSummary    = $("#bhSummary");
const bhModal      = $("#bhModal");
const bhListEl     = $("#bhList");
const bhCancel     = $("#bhCancel");
const bhSave       = $("#bhSave");

const bhDetailModal= $("#bhDetailModal");
const bhDetailTitle= $("#bhDetailTitle");
const bhOpenInput  = $("#bhOpenInput");
const bhCloseInput = $("#bhCloseInput");
const bhDetailCancel=$("#bhDetailCancel");
const bhDetailSave = $("#bhDetailSave");

const bhCopyModal  = $("#bhCopyModal");
const bhCopyList   = $("#bhCopyList");
const bhCopyCancel = $("#bhCopyCancel");
const bhCopyApply  = $("#bhCopyApply");
...
function renderHoursSummary(){
  // Basit bir özet: Pzt–Cum X–Y • Cmt X–Y • Pazar kapalı
  ...
  const sat = bizHours.sat.closed ? "Cumartesi kapalı" : `Cumartesi ${bizHours.sat.open} – ${bizHours.sat.close}`;
  parts.push(sat);
...
openHoursBtn?.addEventListener("click", ()=>{
  hoursBackup = JSON.parse(JSON.stringify(bizHours)); // geri alabilmek için
  renderHoursList();
  openModal(bhModal);
});
...
bhSave?.addEventListener("click", ()=>{
  closeModal(bhModal);
  renderHoursSummary();
  setDirty(true);
});

/* toggle & edit click (delegation) */
bhListEl?.addEventListener("click", (e)=>{
  ...
  if (tgl){
    ...
    if (sum) sum.textContent = bizHours[k].closed ? "Kapalı" : `${bizHours[k].open} - ${bizHours[k].close}`;
  }
  if (ed){
...
function openDayDetail(k){
  currentDetailDay = k;
  bhDetailTitle.textContent = labelOf(k);
  bhOpenInput.value  = bizHours[k].open  || "09:00";
  bhCloseInput.value = bizHours[k].close || "19:00";
  openModal(bhDetailModal);
}
bhDetailCancel?.addEventListener("click", ()=> closeModal(bhDetailModal));

bhDetailSave?.addEventListener("click", ()=>{
  ...
  bizHours[k] = d;
  // ana listedeki özet satırını güncelle
  const row = bhListEl.querySelector(`.bh-item[data-day="${k}"]`);
  row?.querySelector('[data-summary]')?.textContent = d.closed ? "Kapalı" : `${d.open} - ${d.close}`;
  closeModal(bhDetailModal);
  // Kopyala modalını aç
  renderCopyList(k);
...
bhCopyCancel?.addEventListener("click", ()=> closeModal(bhCopyModal));
bhCopyApply?.addEventListener("click", ()=>{
  ...
  bhCopyList.querySelectorAll('[data-copy-target]').forEach(cb=>{
    ...
    if (!bizHours[k].closed){ // kapalı değilse özet de güncellensin
      const row = bhListEl.querySelector(`.bh-item[data-day="${k}"] [data-summary]`);
      if (row) row.textContent = `${src.open} - ${src.close}`;
    }
  });
