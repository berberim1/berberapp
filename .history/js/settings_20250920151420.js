/* ===========================
   ...
   v14.1 — Canonical "businesses" + AO mirror, Staff-sync FIX, images dual-write
   =========================== */

console.log("[settings] settings.js yüklendi (v14.1 canonical + staff-sync)");
...
async function uploadImageToStorage(kind, file, uid) {
  ...
  try {
    ...
    console.warn("[storage] uploadBytes error:", e?.code || e?.message || e);
    if (/appcheck/i.test(e?.code || "") || /AppCheck/i.test(e?.message || ""))
      showToast("Görsel yüklemek için güvenlik doğrulaması (App Check) gerekiyor.", "error");
    else
  ...
  input.addEventListener("change", async () => {
    ...
    for (const f of files) {
      ...
      try {
        ...
        // hata mesajı uploadImageToStorage içinde gösteriliyor
      }
    }
    input.value = null;
  });
...
function formatChangeSummary(changes){
  return changes.map(c=>{
    ...
    if (c.reason === "reset_bounds")
      return `- ${c.name} • ${c.day}: Aralık dışarıdaydı → işletme saatlerine çekildi`;
