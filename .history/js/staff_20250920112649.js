...
console.log("[staff] v26 — BH clamp + realtime + UI rehydrate (tam sürüm)");
...
/* ------------------------- UI modelini yeniden kurucu ------------------------- */
function getDisplayName(s){ return s?.isOwner ? `${s.name} (Admin)` : (s?.name || '—'); }

/** staffRaw + bizHours + topStaffHoursMap'ten staff UI modelini yeniden kurup ekrana basar. */
...
addSave?.addEventListener('click', async ()=>{
  ...
  // yeni kişiyi seçili yapıp UI’yi baştan kur
  currentId = makeId(name, "", newRawIndex);
  rebuildStaffFromRawPreserveSelection();
...
async function clampAllStaffToBusinessHours(newBH){
  ...
  // local state → UI rehydrate (değişiklik olmasa da)
  staffRaw = arr;
  rebuildStaffFromRawPreserveSelection();
...
onAuthStateChanged(auth, async (user)=>{
  ...
  try{
    ...
    /* ---- Realtime: businessHours & staff değişimini izle ---- */
    onSnapshot(rootRef, (snap)=>{
      ...
      // staff array (başka oturumdan değişebilir)
      const remoteStaff = Array.isArray(data?.step8?.staff) ? data.step8.staff
                        : (Array.isArray(data?.staff) ? data.staff : []);
      const remoteMap = data?.step8?.staff_hours || data?.staff_hours || {};

      if (!selfUpdating && JSON.stringify(remoteStaff) !== JSON.stringify(staffRaw)){
        staffRaw = remoteStaff;
        topStaffHoursMap = remoteMap || {};
        rebuildStaffFromRawPreserveSelection();
      }

      if (!selfUpdating && newJSON !== lastBizHoursJSON){
