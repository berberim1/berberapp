...
// staff.js — renderStaffList (minimal)
function renderStaffList(){
  const ul = $('#staffList'); if (!ul) return;
  const owner = $('#ownerList'); if (owner) owner.style.display = 'none'; // tek liste

  ul.innerHTML = '';

  // Not: Admin zaten rebuildStaffFromDocsPreserveSelection() içindeki sort ile en üstte
  staff.forEach(s => {
    const li = document.createElement('li');
    ...
    li.innerHTML = `
      ...
      <div>
        <div>${s.isOwner ? `${s.name} (Admin)` : (s.name || '—')}</div>
    ...
    li.addEventListener('click', () => {
      currentId = s.id;
      $('#panelTitle').textContent = s.isOwner ? `${s.name} (Admin)` : (s.name || '—');
      $('#staffName').textContent  = $('#panelTitle').textContent;
      const n = $('#srvStaffName'); if (n) n.textContent = $('#panelTitle').textContent;
      snapshotAssignedToTemp();
      renderTabs();
      renderStaffList();
    });
