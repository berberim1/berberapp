...
console.log("[staff] v21 (Sahip seçeneği kaldırıldı + staff_hours + hizmet atama + TR 24s)");
...
async function mountRail(){
  try{
    ...
    if(nav){
      ...
      $$('nav.rail .rail__btn').forEach(a => { if (a.getAttribute('href')===current) a.setAttribute('aria-current','page'); });
...
let staffRaw = [];
let staff = [];   // {id,name,role,rawIndex,hours,services:Set<string>}
let bizHours = null; // {sun..sat}
let currentId = null;
let activeTab = 'services';

/* Hizmetler */
let servicesAll = []; // normalized: {id,name,category,price,duration,active}
let serviceCategories = []; // string[]
let srvQuery = "";
let srvCat = "";
let srvTempAssigned = new Set(); // geçici (UI) seçim
...
/* ---- saat kaynağını esnekçe okuyan yardımcılar ---- */
function normalizeAnyHours(src){
  // src: {workingHours|weeklyHours|hours|staff_hours|<TR gün objesi>}
...
function renderStaffList(){
  ...
  staff.forEach(s=>{
    ...
    li.innerHTML = `<div class="avatar">${initial}</div>
                    <div><div>${s.name||'—'}</div><div class="role">${s.role || s.position || 'Personel'}</div></div>`;
    li.addEventListener('click',()=>{
      currentId=s.id;
      $('#panelTitle').textContent=s.name||'—';
      $('#staffName').textContent=s.name||'—';
      $('#srvStaffName') && ($('#srvStaffName').textContent = s.name || '—');
...
function currentStaff(){
  return staff.find(s => s.id === currentId);
}
function snapshotAssignedToTemp(){
  ...
  $('#srvStaffName') && ($('#srvStaffName').textContent = who?.name || '—');
}
...
async function saveServiceAssignments(){
  ...
  if (idx >= 0){
    ...
    arr.push({ name: who.name || "Admin", role: who.role || "Sahip", position: who.role || "Sahip", hours: denormalizeHours(who.hours), services: assigned });
...
function renderStaffHoursView(){
  ...
  $('#panelTitle').textContent = who?.name || '—';
  $('#staffName').textContent  = who?.name || '—';
}
...
$('#saveModal')?.addEventListener('click', async ()=>{
  ...
  try{
    if (who.rawIndex >= 0) {
      ...
      arr.push({ name: who.name || "Admin", role: who.role || "Sahip", position: who.role || "Sahip", hours: payload, services: Array.from(who.services || []) });
      who.rawIndex = arr.length - 1;
    }
    staffRaw = arr;
...
/* ------------------------- ADD STAFF (isim + pozisyon + saatler) ------------------------- */
...
// sayfa yüklenince bir kez temizlik
...
function openAddModal(){
  ...
  removeOwnerOptionFromRoleSelect(asPos); // güvence
  renderAddHoursGrid();
...
addSave?.addEventListener('click', async ()=>{
  ...
  // HER İHTİMALE KARŞI: Sahip/Owner/Admin seçimine izin verme
  if (['sahip','owner','admin'].includes(pos.toLowerCase())) {
    pos = 'Berber'; // istersen 'Personel' yap
  }
  ...
  const newStaff = { id: makeId(name, "", newRawIndex), name, role: pos, position: pos, rawIndex: newRawIndex, hours: newHours, services: new Set(servicesInit) };
  staff.push(newStaff);
...
function buildDelList(){
  ...
  staff.forEach(s=>{
    ...
    const isOwner = ((s.role || s.position || '').toLowerCase() === 'sahip');
    const li = document.createElement('li');
    const id = `del-${s.id}`;
    ...
    li.innerHTML = `
      <label for="${id}" style="display:flex; align-items:center; gap:8px; cursor:pointer;">
        ...
        <span>${s.name}</span>
        <span class="muted" style="margin-left:auto">${s.role || s.position || 'Personel'}${isOwner ? ' • Sahip' : ''}</span>
      </label>
    `;
...
delConfirm?.addEventListener('click', async ()=>{
  ...
  let rebuilt = staffRaw.map((s,idx)=>({
    ...
    phone: s.phone || "",
    rawIndex: idx,
    hours: normalizeAnyHours(s),
    services: new Set(Array.isArray(s.services) ? s.services : (Array.isArray(s.assignedServices) ? s.assignedServices : [])),
  }));

  if (!rebuilt.some(s => (s.role||'').toLowerCase()==='sahip')){
    rebuilt.unshift({ id: makeId(ADMIN_NAME, "", "0"), name: ADMIN_NAME, role: "Sahip", rawIndex: -1, hours: normalizeHours({}), services: new Set() });
  }
...
onAuthStateChanged(auth, async (user)=>{
  ...
  try{
    ...
    if(!rootSnap.exists()){
      staffRaw = [];
      ADMIN_NAME = "Admin";
      staff = [{ id:"admin", name:ADMIN_NAME, role:"Sahip", rawIndex:-1, hours: normalizeHours({}), services:new Set() }];
    ...
    // Admin adı
    ADMIN_NAME = (root?.step2?.adminName || "Admin").trim();

    // Admin/Sahip yoksa ekle
    if (!staff.some(s => (s.role||'').toLowerCase()==='sahip' ||
                         (s.name||'').toLowerCase()===ADMIN_NAME.toLowerCase())){
      staff.unshift({ id: makeId(ADMIN_NAME, "", "0"), name: ADMIN_NAME, role: "Sahip", rawIndex: -1, hours: normalizeHours({}), services: new Set() });
    }
    // Duplikeleri temizle
    const seen = new Set();
    staff = staff.filter(s=>{
      const key = (s.name||'').toLowerCase() + '|' + (s.role||'');
