...
onAuthStateChanged(auth, async (user)=>{
  ...
  try {
    ...
    // loadFromFirestore içinde okuduğumuz adminOnboarding dokümanını tekrar alıp işletme adını yakala
    ...
    setBusinessModalData({
      name: OWNER,          // loadFromFirestore içinde belirlenen kanonik sahip adı
      email: userEmail,
      business: bizName
    });
  } catch (e) {
...
function renderDay(){
  ...
  // ======== KOLON GENİŞLİKLERİ: MOBİLDE 2 KİŞİ, MASAÜSTÜ ESKİ ========
  const isMobile = window.matchMedia('(max-width: 768px)').matches;
  if (isMobile) {
    // görünür alan ölç → (takvim genişliği - zaman sütunu) / 2
    const cal = document.querySelector('.calendar');
    const calW = (cal?.clientWidth || 0);
    const timeW = cssVarPx('--timeW', document.documentElement) || 56; // fallback
    const colW = Math.max(160, Math.floor((calW - timeW) / 2)); // güvenli alt sınır

    row.style.gridTemplateColumns  = `var(--timeW) repeat(2, ${colW}px)`;
    grid.style.gridTemplateColumns = `var(--timeW) repeat(2, ${colW}px)`;
    row.style.gridAutoColumns      = `${colW}px`;
    ...
    row.style.removeProperty('grid-auto-columns');
...
/* =============== PROFILE / BUSINESS MODAL DATA =============== */
function setBusinessModalData({ name, email, business }) {
  const avatarEl   = document.getElementById("bmAvatar");
  ...
  const initials = (txt="") => txt.trim().split(/\s+/).map(w=>w[0]).slice(0,2).join("").toUpperCase() || "A";

  if (avatarEl) avatarEl.textContent = initials(name || email || "A");
  if (nameEl)   nameEl.textContent   = name || "—";
  if (mailEl)   mailEl.textContent   = email || "—";
  if (bizEl)    bizEl.textContent    = business || ""; // boşsa satır görünür kalabilir, sorun değil
}
