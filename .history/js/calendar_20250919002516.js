...
function openModal({ id, overlayId, panelClassOpen='open', ariaTargetId }) {
  closeAllModals();
  // artık backdrop yok (arkayı karartmıyoruz)
...
function renderWeek(){
  ...
  // —— Varsayılan 2 satır görünsün, fazlasında dikey kaydırma
...
dateWrap?.addEventListener("click",()=>{
  ...
  if (willOpen) {
    ...
    drawMini(calCursor);
    // eskiden: backdrop açılırdı — kaldırıldı
  }
});
document.addEventListener("mousedown",(e)=>{
  if(pop?.classList.contains("open") && !pop.contains(e.target) && !dateWrap.contains(e.target)){
    ...
    // eskiden: backdrop kapanırdı — kaldırıldı
  }
});
