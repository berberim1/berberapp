...
console.log("[staff] v24 — BusinessHours değişimini izleme + personel saatleri dükkan sınırına otomatik uyum");
...
let bizHours = null; // {sun..sat} normalized
let currentId = null;
let activeTab = 'services';

/* İşlevsel değişiklikleri yönetmek için */
let lastBizHoursJSON = null;   // stringified normalized business hours
let selfUpdating = false;      // onSnapshot döngüsünü kırmak için
...
/* ------------------------- Firestore mappers ------------------------- */
// Tüm şemaları destekle: {closed, open, close} | {open:boolean, from,to} | {enabled,start,end}
...
function toFirestoreDay(model){
  // adminOnboarding/step8.staff için TR key + open/from/to formatı
  return model.enabled ? { open:true, from:model.start, to:model.end } : { open:false };
}
function normalizeHours(src){
  const out = {};
  DAYS.forEach(d=>{
    const tr = src?.[d.tr];      // Pazartesi
    const en = src?.[d.key];     // mon
...
function normalizeAnyHours(src){
  // personel objesinin içinden saat bloğunu bul
...
/** DÜKKAN SINIRI: start >= open && end <= close && start < end */
function enforceBusinessBoundsForRow(row){
  ...
  // Başlangıç alt sınır, bitiş üst sınır
...
/* ==========================
   DÜKKAN SAATİ DEĞİŞİMİNDE PERSONELİ OTOMATİK SIKIŞTIR
   ========================== */
/** personel modelindeki saatleri (normalized) yeni businessHours'a göre sıkıştırır.
 *  Değişiklik özetini geri döndürür (string[]). */
function clampHoursModelToBiz(model, bh){
  ...
  DAYS.forEach(d=>{
    ...
    if (!m.enabled){
      // personel kapalıysa, kapalı kalmaya devam edebilir (zorlamıyoruz)
      out[d.key] = m;
      return;
    }
    ...
    // halen hatalıysa, dükkana tam eşitle
...
/** Yeni businessHours geldiğinde tüm staffRaw üzerinde clamp uygula ve Firestore'a yaz. */
async function clampAllStaffToBusinessHours(newBH){
  ...
  // Staff array
  arr.forEach((s, idx)=>{
    const currentNorm = normalizeWithFallbackToBiz(s, newBH);
    const { model: clamped, summary } = clampHoursModelToBiz(currentNorm, newBH);

    if (summary.length > 0){
      anyChange = true;
      const name = s?.name || `Çalışan ${idx+1}`;
      allSummaries.push(`- ${name}: ${summary.join(' • ')}`);
    }
    // her durumda objeyi güncelle (normalize->denormalize)
    arr[idx] = { ...(arr[idx]||{}), hours: denormalizeHours(clamped) };
  });

  if (!anyChange){
    // yine de local state'i güncelleyelim
    staffRaw = arr;
    staff.forEach((u, i)=>{
      const src = arr[u.rawIndex >=0 ? u.rawIndex : i] || {};
      u.hours = normalizeWithFallbackToBiz(src, newBH);
    });
    renderStaffList(); renderTabs();
    return;
  }
  ...
  // Kullanıcıya tek bir bilgi penceresi
  alert(
...
/* ------------------------- AUTH → Firestore load + realtime BH watcher ------------------------- */
...
onAuthStateChanged(auth, async (user)=>{
  ...
  try{
    ...
    // 4) Gerekirse business fallback yap
    let needBizFallback   = !bizHours || !DAYS.some(d => (bizHours?.[d.key]?.enabled));
    let needStaffFallback = !Array.isArray(staffRaw) || staffRaw.length === 0;

    if (!rootSnap.exists() || needBizFallback || needStaffFallback || !ADMIN_NAME || ADMIN_NAME === "Admin") {
      try{
        const bizRef  = doc(db, "business", UID);
        const bizSnap = await getDoc(bizRef);
        if (bizSnap.exists()){
          const biz = bizSnap.data();

          if (!ADMIN_NAME || ADMIN_NAME === "Admin") {
            ADMIN_NAME = (biz?.owner?.name || ADMIN_NAME || "Admin").trim();
          }
          if (needBizFallback) {
            bizHours = normalizeHours(biz?.businessHours || biz?.workingHours);
            lastBizHoursJSON = JSON.stringify(bizHours || {});
          }
          if (needStaffFallback && Array.isArray(biz?.staff)) {
            // business.staff: [{ name, position, phone, workingHours, ... }]
            staffRaw = biz.staff;
          }
        }
      }catch(e){
        console.warn("[business fallback] okunamadı:", e?.message || e);
      }
    }

    // 5) Staff UI modelini kur (saatler boşsa dükkan saatlerine düş)
    ...
    // 6) Owner’ı listenin başına ekle (Admin etiketiyle)
    ...
    // 7) Üst seviye staff_hours varsa saatleri override et
    ...
    /* ---- Realtime: businessHours değişimini izle ve personeli sıkıştır ---- */
    onSnapshot(rootRef, (snap)=>{
      if (!snap.exists() || selfUpdating) return;
      ...
      const newJSON = JSON.stringify(newBH || {});
      if (newJSON !== lastBizHoursJSON){
        lastBizHoursJSON = newJSON;
        bizHours = newBH || normalizeHours({});
        // UI için de güncelleyelim
