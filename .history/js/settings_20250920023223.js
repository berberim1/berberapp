/* ===========================
   AYARLAR – Firestore Entegre (v12.1 desktop, modal-hours)
   =========================== */

console.log("[settings] settings.js yüklendi (v12.1 modal-hours)");
...
/* ======================================================
   DÜKKAN SAATLERİ – STATE + MODAL AKIŞLARI (modal tabanlı)
   ====================================================== */
const DAYS = [
  { k: "sun", label: "Pazar" },
...
let BH = defaultHours();
let ACTIVE_DAY = "mon";           // gün detayı modalı için
let COPY_SOURCE_DAY = "mon";      // kopyalama modalı için

/* ---- Genel modal yardımcıları ---- */
function openModal(el){ if(!el) return; el.classList.add("show"); el.setAttribute("aria-hidden","false"); document.body.style.overflow="hidden"; }
...
function bindClosableOverlay(root){
  root?.addEventListener("click", (e)=>{
    if (e.target.matches(".picker-ov,[data-close]")) closeModal(root);
  });
  document.addEventListener("keydown",(e)=>{
    if (e.key==="Escape" && root?.classList.contains("show")) closeModal(root);
  });
}

/* ---- Özet metin (kart üstü) ---- */
const bhSummaryEl = $("#bhSummary");
function same(a,b){return a.open===b.open && a.close===b.close && a.closed===b.closed;}
function summaryForDay(dk){
  const d = BH[dk]; if(!d) return "";
  return d.closed ? "Kapalı" : `${d.open}–${d.close}`;
}
function renderBhSummary(){
  if (!bhSummaryEl) return;
  const weekdaySame = WDAYS.every(k => same(BH[k], BH["mon"]));
  const parts = [];
  if (weekdaySame) {
    parts.push(`Hafta içi ${summaryForDay("mon")}`);
  } else {
    parts.push(`Pzt ${summaryForDay("mon")}, Sal ${summaryForDay("tue")}, Çar ${summaryForDay("wed")}, Per ${summaryForDay("thu")}, Cum ${summaryForDay("fri")}`);
  }
  parts.push(`Cumartesi ${summaryForDay("sat")}`);
  parts.push(`Pazar ${summaryForDay("sun")}`);
  bhSummaryEl.textContent = parts.join(" • ");
}

/* ---- Ana liste render (bhModal içi) ---- */
const bhModal = $("#bhModal");
const bhList = $("#bhList");
const openBhBtn = $("#openBhModal");
bindClosableOverlay(bhModal);

function renderBhList(){
  if (!bhList) return;
  $$("#bhList .bh-item").forEach(row=>{
    const k = row.dataset.day;
    const sw = row.querySelector('input[data-toggle="open"]');
    const sum = row.querySelector('[data-summary]');
    if (!BH[k]) return;
    sw.checked = !BH[k].closed;
    sum.textContent = BH[k].closed ? "Closed" : `${BH[k].open} – ${BH[k].close}`;
    row.classList.toggle("is-closed", !!BH[k].closed);
  });
}
openBhBtn?.addEventListener("click",()=>{
  renderBhList(); renderBhSummary(); openModal(bhModal);
});

/* Switch değişimi (açık/kapalı) */
bhList?.addEventListener("change", (e)=>{
  const t = e.target;
  if (!t.matches('input[data-toggle="open"]')) return;
  const row = t.closest(".bh-item"); const k = row?.dataset.day;
  if (!k) return;
  BH[k].closed = !t.checked;
  renderBhList(); renderBhSummary(); setDirty(true);
});

/* ---- Gün detayı modalı ---- */
const bhDayModal = $("#bhDayModal");
const dayOpenEl = bhDayModal?.querySelector('[data-day-open]');
const dayCloseEl = bhDayModal?.querySelector('[data-day-close]');
const bhDayTtl = $("#bhDayTtl");
bindClosableOverlay(bhDayModal);

/* satırdaki ok butonu ile aç */
bhList?.addEventListener("click",(e)=>{
  const btn = e.target.closest("[data-open-day]");
  if (!btn) return;
  const k = btn.dataset.openDay;
  openDayEditor(k);
});

function openDayEditor(k="mon"){
  ACTIVE_DAY = k;
  const meta = DAYS.find(d=>d.k===k);
  if (bhDayTtl) bhDayTtl.textContent = meta?.label || "Gün";
  if (dayOpenEl) dayOpenEl.value = BH[k].open || "10:00";
  if (dayCloseEl) dayCloseEl.value = BH[k].close || "19:00";
  COPY_SOURCE_DAY = k; // son düzenlenen gün, kopyalama için kaynak kabul edilir
  openModal(bhDayModal);
}

function compareHHMM(a,b){ return (a||"").localeCompare((b||"")); }
function validateDay(open, close){
  if (!open || !close) return false;
  return compareHHMM(open, close) < 0;
}

bhDayModal?.querySelector("[data-save-day]")?.addEventListener("click", ()=>{
  const o = dayOpenEl?.value || "10:00";
  const c = dayCloseEl?.value || "19:00";
  if (!validateDay(o,c)) { showToast("Kapanış açılıştan büyük olmalı", "error"); return; }
  BH[ACTIVE_DAY].open = o;
  BH[ACTIVE_DAY].close = c;
  // Saat güncellendi — gün açık kalsın; kullanıcı switch ile kapatırsa override eder
  renderBhList(); renderBhSummary(); setDirty(true);
  closeModal(bhDayModal);
});

/* ---- Saatleri kopyala modalı ---- */
const bhCopyModal = $("#bhCopyModal");
bindClosableOverlay(bhCopyModal);

bhModal?.querySelector('[data-open="bh-copy"]')?.addEventListener("click", ()=>{
  // kaynak gün: son açılan gün (ACTIVE_DAY) ya da Pazartesi
  COPY_SOURCE_DAY = ACTIVE_DAY || "mon";
  // listeyi resetle (kaynak günü disabled yap)
  $$("#bhCopyList input[type=checkbox]").forEach(cb=>{
    cb.checked = false;
    cb.disabled = (cb.value === COPY_SOURCE_DAY);
  });
  openModal(bhCopyModal);
});

bhCopyModal?.querySelector('[data-apply-copy]')?.addEventListener("click", ()=>{
  const targets = $$("#bhCopyList input[type=checkbox]:checked").map(cb=>cb.value);
  if (!targets.length) { closeModal(bhCopyModal); return; }
  const src = {...BH[COPY_SOURCE_DAY]};
  targets.forEach(k=>{
    BH[k] = {...src};
  });
  renderBhList(); renderBhSummary(); setDirty(true);
...
/* Ana modal footer “Devam” → kapat */
bhModal?.querySelector(".picker__ft [data-close]")?.addEventListener("click", ()=>{
  closeModal(bhModal);
});
...
async function prefillFromDoc(d) {
  ...
  // Saatler (STATE + render)
  BH = { ...defaultHours(), ...data.businessHours };
  renderBhList(); renderBhSummary();
...
/* Saat validasyonu tüm günler için */
function validateHours(obj){
  const key2label = Object.fromEntries(DAYS.map(d=>[d.k,d.label]));
  for (const k of Object.keys(obj)){
    const d = obj[k] || {};
    if (!d.closed){
      if (!d.open || !d.close) return `${key2label[k] || k}: saat giriniz`;
      if (compareHHMM(d.open, d.close) >= 0) return `${key2label[k] || k}: kapanış açılıştan büyük olmalı`;
    }
  }
  return true;
}
...
function buildUpdatesForFirestore() {
  ...
  // ---- businessHours ----
  updates["businessHours"] = BH;
  updates["step6.businessHours"] = BH;
...
const CATALOG = ["Erkek Saç Kesimi","Sakal","Saç & sakal","Çocuk saç kesimi","Saç Boyama","Fön","Saç bakımı","Kaş","Ağda","Perma","Brezilya fönü"];
...
function openPickerModal(){ pickerEl?.classList.add("show"); pickerEl?.setAttribute("aria-hidden","false"); document.body.style.overflow="hidden"; renderPicker(); pickerSearch?.focus(); }
function closePickerModal(){ pickerEl?.classList.remove("show"); pickerEl?.setAttribute("aria-hidden","true"); document.body.style.overflow=""; }
openPicker?.addEventListener("click", () => { tempSelected = new Set(); openPickerModal(); });
pickerEl?.addEventListener("click", (e) => { if (e.target.hasAttribute?.("data-close") || e.target === pickerEl) closePickerModal(); });
document.addEventListener("keydown", (e) => { if (!pickerEl?.classList.contains("show")) return; if (e.key === "Escape") closePickerModal(); });
pickerList?.addEventListener("click", (e) => {
  const t = e.target.closest?.("[data-toggle]"); if (!t) return;
  const name = t.dataset.toggle; const was = tempSelected.has(name);
  was ? tempSelected.delete(name) : tempSelected.add(name);
  renderPicker(); showToast(was ? "Kaldırıldı" : "Eklendi", was ? "default" : "success");
});
pickedChips?.addEventListener("click", (e) => { const rm = e.target.dataset?.rm; if (rm) { tempSelected.delete(rm); renderPicker(); showToast("Kaldırıldı"); } });
savePicked?.addEventListener("click", () => {
  let added = 0;
  tempSelected.forEach((n) => {
    if (!services.some((s) => (s.name || "").toLowerCase() === n.toLowerCase())) { services.push({ name: n, min: 30, price: "" }); added++; }
  });
  renderServices(); closePickerModal(); setDirty(true);
  if (added > 0) showToast(`${added} hizmet eklendi`, "success"); else showToast("Değişiklik yok");
});
