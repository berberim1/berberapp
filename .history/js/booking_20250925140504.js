  ...
  rev.innerHTML = `
    ...
    <div class="sumbox" style="margin-top:16px">
      ${cart
        .map(
          (i) =>
            `<div style="display:flex;justify-content:space-between;font-weight:700;padding:8px 0"><span>${
              i.name
            }<br><span class="small">${i.duration}dk</span></span><span>${TL(
              i.price
            )}</span></div>`
        )
        .join("")}
      <div class="sumrow"><span style="font-weight:700">Toplam:</span><span style="font-weight:800">${TL(
        total
      )}</span></div>
    </div>
...
document
  ...
  ?.addEventListener("click", () => showOv("qOv"));
const finish = () => {
  closeOv("qOv");
  closeOv("reviewOv");
  const doneInfo = document.getElementById("doneInfo");
  if (doneInfo)
    doneInfo.textContent = `${fmtDate(selectedDate)} • ${selectedTime}`;
  showOv("doneOv");
};
...
document.getElementById("doneBtn")?.addEventListener("click", () => {
  ["bookOv", "svcOv", "reviewOv", "qOv", "doneOv", "dateOv", "timeOv"].forEach(
    closeOv
  );
});
