// js/firebase.js  (v4.5 — App Check + doğru Storage bucket + emulator opsiyonları)
...
const firebaseConfig = {
  ...
  storageBucket: "my-barber-system.firebasestorage.app", // doğru domain
...
try {
  ...
  if (isLocalhost) {
    // Debug token modu (sadece localhost’ta)
    // Bu token’i konsolda göreceksiniz → Console > App Check > Manage debug tokens kısmına ekleyin
    self.FIREBASE_APPCHECK_DEBUG_TOKEN = true;
    console.log("[app-check] Debug token aktif (localhost).");
  }

  const { initializeAppCheck, ReCaptchaV3Provider } =
    await import("https://www.gstatic.com/firebasejs/10.12.4/firebase-app-check.js");

  // App Check Site Key (Firebase Console → App Check → Web App → reCAPTCHA v3 site key)
...
/* ========= Storage ========= */
// Burada explicit bucket ile çağırıyoruz
