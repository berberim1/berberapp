...
// Onay akışı
// Onay akışı
// ====== Onay akışı (qOv kaldırıldı) ======
document.getElementById('confirmBook')?.addEventListener('click', () => {
  finish();
});

function finish(){
  // review modali kapanır
  closeOv('reviewOv');

  // başarı modali içeriğini güncelle
  const doneInfo  = document.getElementById('doneInfo');
  if (doneInfo) doneInfo.textContent = `${fmtDate(selectedDate)} • ${selectedTime}`;

  const doneTitle = document.querySelector('#doneOv .ttl');
  if (doneTitle) doneTitle.textContent = 'Randevunuz iletilmiştir';

  // isteğe bağlı: küçük toast
  showToast('Randevunuz iletilmiştir');

  // başarı modali aç
  showOv('doneOv');
}

// başarı modalindeki "Tamam" düğmesi
document.getElementById('doneBtn')?.addEventListener('click', () => {
  ['bookOv','svcOv','reviewOv','doneOv','dateOv','timeOv'].forEach(closeOv);
});
