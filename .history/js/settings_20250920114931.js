...
async function persistStaff(nextEmployees){
  ...
  const topMap = {};
  step8List.forEach(s => { topMap[s.name] = s.hours; });
  ...
  if (staffPath === "sub"){
    ...
    nextEmployees.forEach(emp=>{
      ...
      const newHours = denormalizeBack(emp.hours, emp.raw.hours || emp.raw.workHours || emp.raw.workingHours || {});
  ...
  // 2) Uyumluluk alanlarını da güncelle (staff.html bununla override ediyor)
  try{
