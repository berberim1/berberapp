...
console.log("[staff] v31 — businesses kaynaklı (kanonik) + AO mirror, BH clamp + realtime + UI rehydrate");
...
/* Canonical refs (kanonik -> businesses) */
let AO_REF = null;     // adminOnboarding/{uid}  (mirror)
let BIZ_REF = null;    // businesses/{uid}       (canonical)
...
function toFirestoreDay(model){
  // TR day map için (adminOnboarding step8.staff.hours gibi yerler)
...
function normalizeService(s, idx){
  // businesses.services formatı: [{name, min, price}] — min: dakika
...
function extractServicesFromAO(root){
  // Eski onboarding snapshotlardan
  const candidates = [
    root?.services,
    root?.step7?.services,
    root?.step7?.catalog,
...
onAuthStateChanged(auth, async (user)=>{
  ...
  try{
    ...
    // businessHours öncelikli (businesses)
    ...
    // staffRaw (businesses öncelikli)
    staffRaw = Array.isArray(biz?.staff) ? biz.staff
              : (Array.isArray(root?.step8?.staff) ? root.step8.staff
                 : (Array.isArray(root?.staff) ? root.staff : []));

    // 5) üst seviye map
    topStaffHoursMap = biz?.staff_hours || root?.step8?.staff_hours || root?.staff_hours || {};
    ...
    /* ---- Realtime: businesses (kanonik) + AO (yalnızca aynalama için) ---- */
    onSnapshot(BIZ_REF, (snap)=>{
      if (!snap.exists()) return;
      const data = snap.data() || {};

      // business hours
      const newBH = normalizeHours(data.businessHours || data.workingHours);
      const newJSON = JSON.stringify(newBH || {});

      // staff array (başka oturumdan değişebilir)
      const remoteStaff = Array.isArray(data?.staff) ? data.staff : [];
      const remoteMap   = data?.staff_hours || {};
