/* ===========================
   AYARLAR – Firestore + Business Hours MODAL (v13.2 staff-sync FIX)
   =========================== */

console.log("[settings] settings.js yüklendi (v13.2 staff-sync FIX)");
...
function setDirty(v = true) {
  ...
  const b = $("#saveBtn");
  if (b) {
    // Butonu kilitlemeyelim; sadece görsel uyarı
...
// TR/EN şemalarını birlikte yaz + step8 uyumluluk alanları
...
onAuthStateChanged(auth, async (user) => {
  try {
    ...
    // Giriş olduysa “Kaydet” butonunun kilitli kalmamasını garanti et
    const sb = $("#saveBtn");
