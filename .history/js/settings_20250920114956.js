/* ===========================
   AYARLAR – Firestore + Business Hours MODAL (v13.1 staff-sync FIX)
   =========================== */

console.log("[settings] settings.js yüklendi (v13.1 staff-sync FIX)");
...
function denormalizeBack(nObj, raw){
...
/* === TR anahtar + step8 formatına dönüştürücü (UI ile uyum) === */
function toStep8Day(d){
  if (!d || d.off) return { open:false };
  return { open:true, from: d.start || "09:00", to: d.end || "18:00" };
}
...
// İşletme saatlerine göre çalışan aralıklarını düzelt.
// Uygunsa dokunma; taşan uçları sınırla; tamamen uygunsuzsa open–close yap.
function clampStaffToBusiness(bh, employees){
  const changes = []; // {name, day, reason, fromStart, toStart, fromEnd, toEnd}
  const next = employees.map(emp=>{
    if (!emp.hours) return emp;
    const clone = JSON.parse(JSON.stringify(emp));

    DAY_KEYS.forEach(k=>{
      const e = clone.hours[k];           // {start, end, off}
      const b = bh[k];                    // {open, close, closed}
      if (!e || !b) return;
      ...
      if (e.off) return; // izinli ise saygı duyarız
      ...
      // hiç start/end yoksa komple dükkana eşitle
      if (s == null && en == null) {
      ...
      // ayrık / tutarsız aralık (start>close veya end<open vb.) → komple işletme
      ...
      // sıra hatalıysa: tamamen dükkana eşitle (en sağlam ve kısa yol)
      if (s >= en){
        e.start = b.open; e.end = b.close;
...
// TR (Pazartesi..Pazar) ve EN (mon..sun) şemalarını birlikte yaz.
// Ayrıca step8.staff, step8.staff_hours ve staff_hours alanlarını senkron günceller.
async function persistStaff(nextEmployees){
  if (!UID || !nextEmployees?.length) return;

  const rootRef = doc(db,"adminOnboarding",UID);

  // EN şema üret (open/close/closed)
  const enOf = (n) => ({
  ...
  // TR şema (step8 ile uyumlu: open/from/to)
  const trOf = (n) => ({
  ...
  // 1) Kaynağa yaz (alt koleksiyon veya dizi)
  if (staffPath === "sub"){
    const batch = writeBatch(db);
    nextEmployees.forEach(emp=>{
      const ref = doc(db,"adminOnboarding",UID,"staff",emp.id);
      const mergedHours = { ...enOf(emp.hours), ...trOf(emp.hours) }; // iki şema birlikte
  ...
  // 2) Uyumluluk alanları (staff.html burada okuyor)
...
$("#saveBtn")?.addEventListener("click", async () => {
  ...
  try {
    ...
    // 2) Çalışan saatlerini yaz + step8 uyumluluk alanlarını güncelle
    if (nextEmployees) {
