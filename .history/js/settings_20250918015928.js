/* ===========================
   ...
   - Phone input: +90 sabit, sadece 10 rakam
   =========================== */
...
function bindPhoneMask() {
  ...
  // Don’t let IME/composition break mask
  let composing = false;
...
function materializeData(d) {
  ...
  // Telefonu 10 haneye indir (eğer +90 ile kaydedilmişse de çözer)
  const rawPhone = d?.contact?.phone10 || d?.contact?.phone || "";
  const phone10 = onlyDigits10(
    String(rawPhone).replace(/^\+?90/, "") // +90 başı varsa at
  );
  ...
  return {
    ...
    services: uniq,
    about: d?.about || "",
    contact: { phone10, phone: phone10 ? `+90${phone10}` : "" },
  };
}
...
async function prefillFromDoc(d) {
  ...
  if (contactPhoneEl) contactPhoneEl.value = data.contact.phone10 || "";
...
function buildUpdatesForFirestore() {
  ...
  // About
  if (aboutEl) updates["about"] = (aboutEl.value || "").trim();

  // Phone: sadece 10 hane + E.164
  const phone10 = contactPhoneEl ? onlyDigits10(contactPhoneEl.value) : "";
  if (phone10) {
    updates["contact.phone10"] = phone10;
    updates["contact.phone"] = `+90${phone10}`;
  } else {
    // boş ise alanı boş kaydet (istersen silme)
    updates["contact.phone10"] = "";
...
saveBtnEl?.addEventListener("click", async () => {
  ...
  // Telefon validasyonu: 10 hane şart
  const phone10 = contactPhoneEl ? onlyDigits10(contactPhoneEl.value) : "";
  if (contactPhoneEl && phone10 && phone10.length !== 10) {
    contactPhoneEl.setAttribute("aria-invalid", "true");
