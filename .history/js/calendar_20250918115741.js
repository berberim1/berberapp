...
function renderWeek(){
  ...
  // --- YÜKSEKLİK: ekranda net 2 personel satırı görünsün, fazlası scroll ---
  const wkView = $("#weekView");
  const headH  = $("#wkHead").getBoundingClientRect().height || 0;

  // CSS değişkeninden satır yüksekliği (fallback 150)
  const rowH = parseFloat(
    getComputedStyle(document.documentElement).getPropertyValue('--wkRowH')
  ) || 150;

  const twoRowsHeight = headH + 2*rowH + 16; // küçük tampon

  // maxHeight yerine direkt height veriyoruz ki alanı tam kullansın
  wkView.style.height = px(twoRowsHeight) + "px";
  wkView.style.overflowY = (selectedStaff.length > 2) ? "auto" : "hidden";
