...
function makeId(name, phone, idx){
  ...
  // phone yoksa phoneE164’i da deneriz
...
onAuthStateChanged(auth, async (user)=>{
  ...
  try{
    ...
    bizHours   = normalizeHours(root?.step6?.workingHours || root?.workingHours);
    staffRaw   = Array.isArray(root?.step8?.staff) ? root.step8.staff
               : (Array.isArray(root?.staff) ? root.staff : []);

    // 4) Gerekirse business fallback yap (Calendar'ın kullandığı kaynak)
    ...
    if (!rootSnap.exists() || needBizFallback || needStaffFallback || !ADMIN_NAME || ADMIN_NAME === "Admin") {
      try{
        ...
        if (bizSnap.exists()){
          ...
          if (needBizFallback) {
            bizHours = normalizeHours(biz?.workingHours);
          }
