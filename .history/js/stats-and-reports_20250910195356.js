...
async function getYearMonthlyTotals(year){
  ...
  for(let m=0;m<12;m++){
    const sales = await getSalesByMonth(year, m);
    totals[m] = sales.reduce((s,r)=> s + Number(r.total||0), 0);
    counts[m] = sales.length; // satış= randevu varsayımı
  }
...
/* =========================================================
   DOM’a veri basma (örnek içerikleri tamamen değiştiriyoruz)
   ========================================================= */
  ...
  if(t3){
    ...
    [...staffMap.entries()].forEach(([name,v])=>{
      ...
      tr.innerHTML = `
        ...
        <td>${Math.floor(v.minutes/60)}s ${v.minutes%60}dk</td>
        <td>72s 0dk</td>`;
      t3.appendChild(tr);
    });
  }
