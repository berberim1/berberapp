...
console.log("[staff] v31.2 — businesses kanonik + AO mirror, BH clamp + realtime + UI rehydrate (upsert & day-normalize fixes)");
...
function normalizeDay(src){
  ...
  // start/end çıkarımı — DİKKAT: src.open boolean olabilir, saat değildir.
...
onSnapshot(BIZ_REF, (snap) => {
  ...
  if (!selfUpdating && newJSON !== lastBizHoursJSON) {
    ...
    if (AUTO_CLAMP) {
      // personel saatlerini dükkan saatine sıkıştır
      clampAllStaffToBusinessHours(bizHours);
    } else {
      // clamp yoksa sadece UI’yi yeni BH ile güncelle
      rebuildStaffFromRawPreserveSelection();
      renderTabs(); // aktif sekme staff hours ise ekranda yenilenir
    }
  }
});
